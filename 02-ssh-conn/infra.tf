...
provisioner "remote-exec" {
    inline = [
         ...
         "sudo chmod 600 /home/centos/.ssh/id_rsa  /home/centos/.ssh/config",
...
output "instance_ip_addr" {
  value = aws_instance.batch34.private_ip
}

resource "local_file" "foo" {
    content     = "${aws_instance.batch34.private_ip}"
    filename = "/tmp/hosts-terraform"
}
